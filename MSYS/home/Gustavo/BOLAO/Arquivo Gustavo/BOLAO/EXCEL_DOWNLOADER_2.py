...
for row in range(len(array)):
	...
	if row == 0:
		for col in range(len(array[row])):
			...
			if col <= 3:
				continue
			elif array[row][col] == "Endereço de e-mail":
				continue
			elif col % 2 == 0:
		...
		continue
	for i, col in enumerate(array[row]):
		if col < (len(array[row])) and i != 1: #Tirei o -1 pq agora o último campo nao é o endereço de email
			infos.append(array[row][col])

	# print(f"{data0[infos[0]]}")
	sheet = workbook[f"{data0[infos[1]]}"]
	for x in range(5000):
		if sheet[('A'+str(x+1))].value == data['Nome_Rodada']:
			position = ('A'+str(x+1))
