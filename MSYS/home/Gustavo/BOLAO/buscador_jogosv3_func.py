...
def main_buscador_jogosv3(dt, dt2, url):
	...
	def checking_rescheduling(string):
		...
		try:
			...
			for tags in x:
				try:
					...
					if v == 1:
						# pdb.set_trace()
						if checking_rescheduling(y) == "postponed":
	...
	if 'inicio_bolao' in locals():
		# print(inicio_bolao)
		returnVariable.append(inicio_bolao)
	# pdb.set_trace()
	...
	# if f == 1:
		if (j1 == 1 and (("europa-league" in url and "europa-league-qual" not in url) or ("champions-league" in url and "champions-league-qual" not in url))) or ( j1 == 2 and (("europa-conference-league" in url and "europa-conference-league-qual" not in url) or ("europa-league" in url and "europa-league-qual" not in url) or ("champions-league" in url and "champions-league-qual" not in url))):
			for index, y in enumerate(jogos_semana):
				try:
					# print(y)
					returnVariable.append(y)
				except:
					returnVariable.append("Time com caracteres especiais, buscar manualmente")
			...
			if j1==1:
				if len(jogos_semana) > 0:
					# print("j1=1")
					returnVariable.append("j1=1")

			elif j1==2:
				if len(jogos_semana) > 0:
					# print("j1=2")
					returnVariable.append("j1=2")
